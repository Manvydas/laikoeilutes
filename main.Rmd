...
plot(nusez, type = "l", main = "Prognozės vs tikrosios reikšmės", lwd = 3, ylab = "", xlab = "Laikas")
lines(progn1, col = "red", lwd = 3)
legend("bottomright", c("Tikrosios reikšmės", "Prognozės"), col = c(1,2), lty = c(1,1), cex = 0.8, lwd = 3, pt.cex = 1, text.width = 1)
```

Kryžminės patikros būdu gautos prognozės atrodo ganėtinai tikslios. Palyginsime gauto modelio prognozių ir $auto.arima$ modelio prognozių tikslumą pagal RMSE, MAE, MAPE.

```{r}
mod.arima <- auto.arima(nusez)
f2 = rep(NA, 94)
for(i in 11:94) try({
  mod <- arima(nusez[1:(i-1)], order = mod.arima$arma[c(1, 6, 2)], include.mean = F)
  fc = forecast(mod, h=1)
  f2[i] <- fc$mean
}, silent = T)
